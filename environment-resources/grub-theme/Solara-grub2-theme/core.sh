...
copy_files() {
  ...
  cp -a --no-preserve=ownership "${REO_DIR}/backgrounds/background-${position}.jpg" "${THEME_DIR}/background.jpg"
  ...
  cp -a --no-preserve=ownership "${REO_DIR}/config/theme-${position}-${screen}.txt" "${THEME_DIR}/theme.txt"
  ...
  cp -a --no-preserve=ownership "${REO_DIR}/assets/assets-other/other-${screen}/info-${position}.png" "${THEME_DIR}/info.png"
  ...
  if [[ -f "${REO_DIR}/background.jpg" ]]; then
    ...
    magick -auto-orient "${THEME_DIR}/background.jpg" "${THEME_DIR}/background.jpg"
  fi
}
...
install() {
  ...
  THEME_DIR="${GRUB_DIR}/${THEME_NAME}-${position}"
  ...
  if [[ "$UID" -eq "$ROOT_UID" ]]; then
    ...
    # Set theme
    prompt -i "Setting ${THEME_NAME}-${position} as default...\n"
  ...
  #Check if password is cached (if cache timestamp has not expired yet)
    ...
    if [[ "${install_boot}" == 'true' ]]; then
      sudo "$0" -p ${position} -s ${screen} -l ${logo} -b
    else
      sudo "$0" -p ${position} -s ${screen} -l ${logo}
    fi
  else
    #Ask for password
    if [[ -n ${tui_root_login} ]] ; then
      if [[ -n "${theme}" && -n "${screen}" ]]; then
        if [[ "${install_boot}" == 'true' ]]; then
          sudo -S $0 -p ${position} -s ${screen} -l ${logo} -b <<< ${tui_root_login}
        else
          sudo -S $0 -p ${position} -s ${screen} -l ${logo} <<< ${tui_root_login}
        fi
      fi
    else
      ...
      if sudo -S echo <<< $REPLY 2> /dev/null && echo; then
        ...
        if [[ "${install_boot}" == 'true' ]]; then
          sudo -S "$0" -p ${position} -s ${screen} -l ${logo} -b <<< ${REPLY}
        else
          sudo -S "$0" -p ${position} -s ${screen} -l ${logo} <<< ${REPLY}
        fi
      else
...
run_dialog() {
  if [[ -x /usr/bin/dialog ]]; then
    ...
    --radiolist "Choose your Grub theme background image position : " 15 40 5 \
      1 "left" on \
...
remove() {
  ...
  local THEME_DIR="${GRUB_DIR}/${THEME_NAME}-${1}"
  ...
  if [[ "$UID" -eq "$ROOT_UID" ]]; then
    ...
    if [[ -d "${THEME_DIR}" ]]; then
      ...
      rm -rf "${THEME_DIR}"
      prompt -w "\n Removed: '${THEME_DIR}'..."
    elif [[ -d "/boot/grub/themes/${THEME_NAME}-${1}" ]]; then
      prompt -i "\n Find installed theme: '/boot/grub/themes/${THEME_NAME}-${1}'..."
      rm -rf "/boot/grub/themes/${THEME_NAME}-${1}"
      prompt -w "\n Removed: '/boot/grub/themes/${THEME_NAME}-${1}'..."
    elif [[ -d "/boot/grub2/themes/${THEME_NAME}-${1}" ]]; then
      prompt -i "\n Find installed theme: '/boot/grub2/themes/${THEME_NAME}-${1}'..."
      rm -rf "/boot/grub2/themes/${THEME_NAME}-${1}"
      prompt -w "\n Removed: '/boot/grub2/themes/${THEME_NAME}-${1}'..."
    else
      prompt -e "\n Specified ${THEME_NAME}-${1} theme does not exist!"
      exit 0
    fi
    ...
    if sudo -n true 2> /dev/null && echo; then
      ...
      if sudo -S echo <<< $REPLY 2> /dev/null && echo; then
        #Correct password, use with sudo's stdin
        sudo -S "$0" -p ${position} "${PROG_ARGS[@]}" <<< $REPLY
      else
        #block for 3 seconds before allowing another attempt
        sleep 3
        echo -e '\0033\0143'
